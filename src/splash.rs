...
pub fn showWelcome() {
    ...
    \nThis program is intended to run automatically after setup but can be run manually by selecting \"automated-run\" at th menu.
    \nThis program will need to be configured in windows task scheduler after setup by using the attached batch file.";
    println!("{}", message);
}
